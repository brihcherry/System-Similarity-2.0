...
Each pair of systems gets a similarity **score** from **0 to 100**, computed as a **weighted average** across however many of the six categories you have enabled. The default weight for every enabled category is **1** (so with no weights set the score is simply the equal-weight mean). Setting a weight on a category multiplies that category's contribution to the score, letting it dominate or recede in the composite. The six categories are:
...
$$\text{score} = \frac{\sum_{i=1}^{N} w_i \cdot \text{categoryScore}_i}{\sum_{i=1}^{N} w_i} \quad \text{(default } w_i = 1\text{)}$$

For example, with all six categories enabled and a weight of `50` on Environment (and the other five defaulting to `1`), Environment contributes roughly 91% of the composite (`50 / (50 + 5)`).
...
Re-runs the similarity computation with the categories and per-category weights you choose. It does not reload the page.

To use it:

1. In the Refresh Heatmap section, check or uncheck the six categories to include or exclude them.
2. Optionally, type an integer in the **Weight (optional integer)** box next to any checked category. The value is a **multiplier** on that category's contribution to the composite score. A blank box keeps the default weight of `1`. Larger values make the category dominate; `0` removes its contribution. Weights do not exclude any pair on their own — they only change how scores are computed.
...
|---|---|
| Login credentials are wrong | Inline message under the password field: `Username or password is incorrect.` |
...
| Term | Definition |
|---|---|
| Score | A 0–100 number for one pair of systems, equal to the weighted average of the per-category scores for the categories included in the current run. The default weight for every category is 1, so with no weights set this reduces to the equal-weight mean. |
